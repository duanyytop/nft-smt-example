...
fn generate_claimed_compact_nft_from_mint(
    ...
    for index in 0..claim_leaves_count {
        ...
        println!("input_compact_nft_out_point: {:?}", &input_compact_nft_out_point.as_slice()[12..]);
...
fn test_claim_from_mint() {
    ...
    let index = [0u8, 0u8, 0u8, 1u8].map(|v| nft_smt::ckb_types::packed::Byte::from(v));
    ...
    let class_mint_proof = mint_entries.proof().as_slice().to_vec();
